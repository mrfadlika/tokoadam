        ...
        <form method="POST" action="<?= BASE_URL ?>/index.php?page=sales&action=store" id="saleForm">
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Pilih Pelanggan <span class="required">*</span></label>
                    <input type="text" id="customerSearch" class="form-control" placeholder="Ketik nama pelanggan..." autocomplete="off">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    custSearch.addEventListener('input', function() {
        ...
        custTimer = setTimeout(() => {
            fetch(BASE + '/index.php?page=customers&action=api_search&type=customer&q=' + encodeURIComponent(q))
                .then(r => r.json())
                .then(data => {
                    if (!data.length) {
                        custResult.innerHTML = '<div class="autocomplete-dropdown"><div class="dd-item" style="color:var(--text-muted)">Tidak ditemukan</div></div>';
                        return;
                    }
                    let html = '<div class="autocomplete-dropdown">';
                    data.forEach(c => {
                        html += '<div class="dd-item" data-id="' + c.id + '" data-name="' + c.nama_toko + '">' + c.nama_toko + ' <small>' + (c.nama_pic || '') + '</small></div>';
                    });
                    html += '</div>';
    ...
    function addItemRow() {
        ...
        tr.innerHTML = `
            ...
            <td><button type="button" class="btn btn-sm btn-icon btn-outline remove-row" data-idx="${idx}">X</button></td>
        `;
        ...
        searchInput.addEventListener('input', function() {
            ...
            pTimer = setTimeout(() => {
                fetch(BASE + '/index.php?page=products&action=api_search&q=' + encodeURIComponent(q))
                    ...
                    .then(data => {
                        if (!data.length) {
                            dropdown.innerHTML = '<div class="autocomplete-dropdown"><div class="dd-item" style="color:var(--text-muted)">Tidak ditemukan</div></div>';
                            return;
                        }
                        let html = '<div class="autocomplete-dropdown">';
                        data.forEach(p => {
    ...
    document.getElementById('saleForm').addEventListener('submit', function(e) {
        if (!custId.value) { e.preventDefault(); alert('Pilih pelanggan terlebih dahulu.'); return; }
